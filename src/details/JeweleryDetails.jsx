...
const JewleryDetails = () => {
    const [jewlery, setJewelery] = React.useState(null)
  ...
  return (
      jewlery && 
      <>
      <Link to="/categories/jeweleries"><p className="flex items-center m-4 md:m-8"><IoChevronBackSharp /> Go back</p></Link>     
        <div key={jewlery.id} id={jewlery.id}
            ...
            <img className="w-full h-32 sm:h-40 md:h-48 object-contain"
                src={jewlery.image} alt={jewlery.title} />
            
            <p className="font-bold text-sm md:text-base p-2 sm:px-4">{jewlery.title}</p>
            <p className="text-sm md:text-base font-medium px-1 sm:px-2">
                PLN {jewlery.price.toFixed(2)}
            </p>
            <p className="text-xs sm:text-sm p-2 sm:p-4">{jewlery.description}</p>
            <button className="bg-amber-600 text-white p-2 border m-2 md:m-4 rounded w-full md:w-48 cursor-pointer hover:bg-yellow-500 focus:ring-yellow-300 transition-colors">Buy Now</button>
        </div>
        </>
  )
}

export default JewleryDetails