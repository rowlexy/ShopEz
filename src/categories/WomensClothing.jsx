...
const WomensClothing = () => {
  ...
  const renderWomensClothing = womensClothing.map((item) => {
    return (
      <div
        key={item.id}
        id={item.id}
      ...
      >
        ...
        <p className="text-sm md:text-base font-medium px-1 sm:px-2">
         ...
         <Link to={`/categories/womens-clothing/${item.id}`}><button key={item.id} id={item.id} 
         className={`${categoryColor(item.category)} text-sm mx-1 my-2 md:m-2 hover:bg-pink-300 active:bg-pink-300 md:text-base text-white cursor-pointer 
         w-full md:w-48 p-2 rounded transition-all`}>Buy Now</button></Link>
      </div>
    );
  });
  return (
